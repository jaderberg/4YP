function [conf, imdb] = db_full_helper()
% DB_HELPER

%   rootDir = fileparts(mfilename('fullpath')) ;
  rootDir = '/Volumes/4YP/visualindex';
  ...
  conf.imdbPath = fullfile(conf.dataDir, 'imbd_full.mat') ;
  conf.modelPath = fullfile(conf.dataDir, 'model_full.mat') ;
...
% --------------------------------------------------------------
  ...
  for i = 1:length(files)
    ...
    a = regexp(files{i}, '([\w_]+)_[0-9]+_good|ok|junk.txt', 'tokens') ;
    if ~isempty(a),
      cl = a{1} ;
      instances = textread(fullfile(gtDir, files{i}), ...
                           '%s') ;
