function [X, map, alpha] = imreadurl(url, timeout)
%IMREADURL Read image from url using timeout.
...
%   $Revision: 1.1.6.19 $  $Date: 2010/06/07 16:35:25 $
...
if (strfind(filename, '://'))
    ...
    try
        filename = urlwrite2(filename, tempname, [],[],timeout);
    catch %#ok<*CTCH>
...
if ((fmt_s.alpha) && (nargout == 3))
    ...
    [X, map, alpha] = feval(fmt_s.read, filename);
