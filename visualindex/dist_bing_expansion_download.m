...
bing_data_dump_dir = fullfile(conf.rootDir, 'bing_data_dump');
vl_xmkdir(bing_data_dump_dir);

n_image = 1;
for n=1:length(class_names)
    class_name = class_names{n};
    class_dump_dir = fullfile(bing_data_dump_dir, class_name);
    vl_xmkdir(class_dump_dir);
    ...
    for i=1:n_photos
        ...
        frames_file = fullfile(class_dump_dir, [filename '-frames.mat']);
        words_file = fullfile(class_dump_dir, [filename '-words.mat']);
...
%             words if it is spatially verified
        for j=1:length(f_filenames) 
            ...
            if score > opts.matchThresh && score < 250
                ...
                save_figure(1, fullfile(class_report_dir, int2str(j)));
