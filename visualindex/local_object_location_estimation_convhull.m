...
function local_object_location_estimation_convhull( n_split, N_split, first_host, this_host)
    ...
    while coll_ims.hasNext()
        ...
        k = convhull(x,y);
        save(fullfile(object_region_dir, [image_id '-convhull.mat']), 'k');
...
%         plot(x,y,'g.', 'LineWidth', 1);
        hold off;
        if voting
            save_figure(1, fullfile(object_voted_results_dir, [image_id '-hull']));
        else
            save_figure(1, fullfile(object_normal_results_dir, [image_id '-hull']));
        end
